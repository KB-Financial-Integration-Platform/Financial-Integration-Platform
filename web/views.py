...
from web.real_estate.year_size import apt_search
...
아파트_종류 = pd.read_csv('web/real_estate/아파트_종류_단지명포함.csv',encoding='cp949')
...
def 실거래가(request):    
    ...
    if request.method == 'POST':
        ...
        apt_list = 아파트_최종[cond][-10:].rename(columns={'거래금액(만원)':'거래금액'}).reset_index()
        apt25 = int(아파트25[아파트25['단지명']==str(apt)]['0'].mean())
        apt25억 = apt25// 10000
        apt25가격 = apt25 % 10000
        apt_json = apt_list.to_json(orient='records')
        apt_list_json = json.loads(apt_json)
        context = {"apt":apt,"apt25억":apt25억,"apt25가격":apt25가격,'apt_list_json':apt_list_json}
        context['years'],context['price'] = apt_search(apt)
        
        return render(request, '../templates/부동산_실거래가조회.html', context)
    else:
